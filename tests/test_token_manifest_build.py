...
def test_smoke_configs_may_cycle_a_tiny_corpus(tmp_path):
    """The TinyLM harness deliberately wraps, so the epoch bound must not apply to it."""
    tokens = tmp_path / "tokens"
    _write_corpus(tokens)
    manifest = build_manifest(tokens)
    cfg = {
        "smoke": {"sequence_length": 64},
        "data": {"sequence_length": 2048},
        "train": {"max_tokens": 10**9},
    }
    budget = validate_token_budget(cfg, manifest)
    assert budget["cycles_corpus"] is True
    assert budget["sequence_length"] == 64
