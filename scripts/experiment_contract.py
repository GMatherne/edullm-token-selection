"""Fail-closed contracts shared by the two REL experiment arms.
...
def contract_sequence_length(cfg: Mapping[str, Any]) -> int:
    """Sequence length the order contract and dataset agree on (smoke overrides prod)."""
    smoke_len = (cfg.get("smoke") or {}).get("sequence_length")
    data_len = (cfg.get("data") or {}).get("sequence_length")
    seq_len = int(smoke_len or data_len or 0)
    if seq_len <= 0:
...
def validate_token_budget(
    ...
    Smoke configs are exempt: the TinyLM harness cycles a deliberately tiny corpus (see
    ``smoke_data.iter_train_batches``), so a one-epoch bound is not a defect there.
    """
    ...
    cycles_corpus = bool(cfg.get("smoke"))
